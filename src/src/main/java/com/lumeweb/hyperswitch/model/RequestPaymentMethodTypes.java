...
public class RequestPaymentMethodTypes {
  ...
  /**
   * Return true if this RequestPaymentMethodTypes object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
      ...
      for (int i = 0; i < getCardNetworks().size(); i++) {
        if (getCardNetworks().get(i) != null) {
          joiner.add(String.format("%scard_networks%s%s=%s", prefix, suffix,
              ...
              URLEncoder.encode(ApiClient.valueToString(getCardNetworks().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
        }
      }
    }
    ...
    if (getMinimumAmount() != null) {
      joiner.add(String.format("%sminimum_amount%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getMinimumAmount()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `maximum_amount` to the URL query string
    if (getMaximumAmount() != null) {
      joiner.add(String.format("%smaximum_amount%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getMaximumAmount()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
