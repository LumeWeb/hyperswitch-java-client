...
public class SdkInformation {
  ...
  public SdkInformation putSdkEphemPubKeyItem(String key, String sdkEphemPubKeyItem) {
    if (this.sdkEphemPubKey == null) {
      this.sdkEphemPubKey = new HashMap<>();
    }
  ...
  /**
   * Return true if this SdkInformation object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getSdkAppId() != null) {
      joiner.add(String.format("%ssdk_app_id%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getSdkAppId()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `sdk_enc_data` to the URL query string
    if (getSdkEncData() != null) {
      joiner.add(String.format("%ssdk_enc_data%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getSdkEncData()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
    ...
    if (getSdkEphemPubKey() != null) {
      for (String _key : getSdkEphemPubKey().keySet()) {
        joiner.add(String.format("%ssdk_ephem_pub_key%s%s=%s", prefix, suffix,
            ...
            getSdkEphemPubKey().get(_key), URLEncoder.encode(ApiClient.valueToString(getSdkEphemPubKey().get(_key)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }

    // add `sdk_trans_id` to the URL query string
    if (getSdkTransId() != null) {
      joiner.add(String.format("%ssdk_trans_id%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getSdkTransId()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `sdk_reference_number` to the URL query string
    if (getSdkReferenceNumber() != null) {
      joiner.add(String.format("%ssdk_reference_number%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getSdkReferenceNumber()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `sdk_max_timeout` to the URL query string
    if (getSdkMaxTimeout() != null) {
      joiner.add(String.format("%ssdk_max_timeout%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getSdkMaxTimeout()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
