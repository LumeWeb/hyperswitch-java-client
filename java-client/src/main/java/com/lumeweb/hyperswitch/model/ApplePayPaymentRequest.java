...
import com.lumeweb.hyperswitch.model.AmountInfo;
import com.lumeweb.hyperswitch.model.ApplePayAddressParameters;
...
public class ApplePayPaymentRequest {
  ...
  /**
   * Return true if this ApplePayPaymentRequest object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getCountryCode() != null) {
      joiner.add(String.format("%scountry_code%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getCountryCode()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `currency_code` to the URL query string
    if (getCurrencyCode() != null) {
      joiner.add(String.format("%scurrency_code%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getCurrencyCode()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
    ...
    if (getMerchantCapabilities() != null) {
      for (int i = 0; i < getMerchantCapabilities().size(); i++) {
        joiner.add(String.format("%smerchant_capabilities%s%s=%s", prefix, suffix,
            ...
            URLEncoder.encode(ApiClient.valueToString(getMerchantCapabilities().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }
    ...
    if (getSupportedNetworks() != null) {
      for (int i = 0; i < getSupportedNetworks().size(); i++) {
        joiner.add(String.format("%ssupported_networks%s%s=%s", prefix, suffix,
            "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
            URLEncoder.encode(ApiClient.valueToString(getSupportedNetworks().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }
      ...
      for (int i = 0; i < getRequiredBillingContactFields().size(); i++) {
        if (getRequiredBillingContactFields().get(i) != null) {
          joiner.add(String.format("%srequired_billing_contact_fields%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(ApiClient.valueToString(getRequiredBillingContactFields().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
        }
      }
    }
      ...
      for (int i = 0; i < getRequiredShippingContactFields().size(); i++) {
        if (getRequiredShippingContactFields().get(i) != null) {
          joiner.add(String.format("%srequired_shipping_contact_fields%s%s=%s", prefix, suffix,
              "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
              URLEncoder.encode(ApiClient.valueToString(getRequiredShippingContactFields().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
        }
      }
    }
