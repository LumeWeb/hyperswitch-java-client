...
public class SamsungPaySessionTokenResponse {
  ...
  /**
   * Return true if this SamsungPaySessionTokenResponse object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getServiceId() != null) {
      joiner.add(String.format("%sservice_id%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getServiceId()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `order_number` to the URL query string
    if (getOrderNumber() != null) {
      joiner.add(String.format("%sorder_number%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getOrderNumber()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
    ...
    if (getAllowedBrands() != null) {
      for (int i = 0; i < getAllowedBrands().size(); i++) {
        joiner.add(String.format("%sallowed_brands%s%s=%s", prefix, suffix,
            ...
            URLEncoder.encode(ApiClient.valueToString(getAllowedBrands().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }
