...
import com.lumeweb.hyperswitch.model.GpayBillingAddressParameters;
...
public class GpayAllowedMethodsParameters {
  ...
  /**
   * Return true if this GpayAllowedMethodsParameters object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getAllowedAuthMethods() != null) {
      for (int i = 0; i < getAllowedAuthMethods().size(); i++) {
        joiner.add(String.format("%sallowed_auth_methods%s%s=%s", prefix, suffix,
            ...
            URLEncoder.encode(ApiClient.valueToString(getAllowedAuthMethods().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }
    ...
    if (getAllowedCardNetworks() != null) {
      for (int i = 0; i < getAllowedCardNetworks().size(); i++) {
        joiner.add(String.format("%sallowed_card_networks%s%s=%s", prefix, suffix,
            "".equals(suffix) ? "" : String.format("%s%d%s", containerPrefix, i, containerSuffix),
            URLEncoder.encode(ApiClient.valueToString(getAllowedCardNetworks().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
      }
    }

    // add `billing_address_required` to the URL query string
    if (getBillingAddressRequired() != null) {
      joiner.add(String.format("%sbilling_address_required%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getBillingAddressRequired()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
    ...
    if (getAssuranceDetailsRequired() != null) {
      joiner.add(String.format("%sassurance_details_required%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getAssuranceDetailsRequired()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
