...
import com.lumeweb.hyperswitch.model.TransactionStatus;
...
public class PaymentsExternalAuthenticationResponse {
  ...
  /**
   * Return true if this PaymentsExternalAuthenticationResponse object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getTransStatus() != null) {
      joiner.add(String.format("%strans_status%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getTransStatus()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `acs_url` to the URL query string
    if (getAcsUrl() != null) {
      joiner.add(String.format("%sacs_url%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getAcsUrl()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `challenge_request` to the URL query string
    if (getChallengeRequest() != null) {
      joiner.add(String.format("%schallenge_request%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getChallengeRequest()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `acs_reference_number` to the URL query string
    if (getAcsReferenceNumber() != null) {
      joiner.add(String.format("%sacs_reference_number%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getAcsReferenceNumber()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `acs_trans_id` to the URL query string
    if (getAcsTransId() != null) {
      joiner.add(String.format("%sacs_trans_id%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getAcsTransId()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `three_dsserver_trans_id` to the URL query string
    if (getThreeDsserverTransId() != null) {
      joiner.add(String.format("%sthree_dsserver_trans_id%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getThreeDsserverTransId()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `acs_signed_content` to the URL query string
    if (getAcsSignedContent() != null) {
      joiner.add(String.format("%sacs_signed_content%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getAcsSignedContent()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
