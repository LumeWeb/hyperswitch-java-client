...
import com.lumeweb.hyperswitch.model.CardDetailFromLocker;
import com.lumeweb.hyperswitch.model.MaskedBankDetails;
import com.lumeweb.hyperswitch.model.PaymentExperience;
import com.lumeweb.hyperswitch.model.PaymentMethod;
import com.lumeweb.hyperswitch.model.PaymentMethodIssuerCode;
import com.lumeweb.hyperswitch.model.PaymentMethodType;
import com.lumeweb.hyperswitch.model.SurchargeDetailsResponse;
...
public class CustomerPaymentMethod {
  ...
  /**
   * Return true if this CustomerPaymentMethod object is equal to o.
   */
  @Override
  ...
  public String toUrlQueryString(String prefix) {
    ...
    if (getPaymentMethodIssuer() != null) {
      joiner.add(String.format("%spayment_method_issuer%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getPaymentMethodIssuer()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `payment_method_issuer_code` to the URL query string
    if (getPaymentMethodIssuerCode() != null) {
      joiner.add(String.format("%spayment_method_issuer_code%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getPaymentMethodIssuerCode()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `recurring_enabled` to the URL query string
    if (getRecurringEnabled() != null) {
      joiner.add(String.format("%srecurring_enabled%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getRecurringEnabled()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `installment_payment_enabled` to the URL query string
    if (getInstallmentPaymentEnabled() != null) {
      joiner.add(String.format("%sinstallment_payment_enabled%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getInstallmentPaymentEnabled()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
      ...
      for (int i = 0; i < getPaymentExperience().size(); i++) {
        if (getPaymentExperience().get(i) != null) {
          joiner.add(String.format("%spayment_experience%s%s=%s", prefix, suffix,
              ...
              URLEncoder.encode(ApiClient.valueToString(getPaymentExperience().get(i)), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
        }
      }
    }
    ...
    if (getRequiresCvv() != null) {
      joiner.add(String.format("%srequires_cvv%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getRequiresCvv()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `last_used_at` to the URL query string
    if (getLastUsedAt() != null) {
      joiner.add(String.format("%slast_used_at%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getLastUsedAt()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }

    // add `default_payment_method_set` to the URL query string
    if (getDefaultPaymentMethodSet() != null) {
      joiner.add(String.format("%sdefault_payment_method_set%s=%s", prefix, suffix, URLEncoder.encode(ApiClient.valueToString(getDefaultPaymentMethodSet()), StandardCharsets.UTF_8).replaceAll("\\+", "%20")));
    }
